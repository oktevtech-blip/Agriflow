...
import { useState, useMemo } from 'react';
import { Users, Plus, Phone, MapPin, Search, Wheat } from 'lucide-react';
import { store } from '@/lib/store';
import { useAgriData } from '@/lib/hooks';
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          subtitle="Try adjusting your search or add a new farmer."
        />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((farmer) => (
            ...
            >
              ...
              <div className="flex items-start gap-3 mb-5">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-display font-bold text-earth-900 truncate group-hover:text-brand-700 transition-colors">
                    {farmer.name}
                  </h3>
              ...
              <div className="space-y-3">
                ...
                {/* Crop */}
                <div className="flex items-center gap-2 text-sm text-earth-600">
                  <Wheat size={15} className="text-brand-600" />
                  <span>
                    {farmer.crop || 'No crop specified'}
                  </span>
                </div>
              </div>
            </button>
...
function AddFarmerModal({
  ...
  const submit = async () => {
    if (!form.name || !form.phone || !form.location || !form.crop) {
      setError('Full name, phone number, location and crop are required.');
      return;
    }
...
function FarmerDetailModal({
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={farmer.name}
      subtitle="Farmer Information"
    >
      <div className="space-y-4">
        {/* Full Name */}
        <div className="rounded-xl bg-earth-50 p-4">
          <div className="text-xs text-earth-500 font-medium mb-1">
            Full Name
          </div>
        ...
        <div className="rounded-xl bg-earth-50 p-4">
          ...
          <div className="font-semibold text-earth-900 flex items-center gap-2">
        ...
        <div className="rounded-xl bg-earth-50 p-4">
          ...
          <div className="font-semibold text-earth-900 flex items-center gap-2">
            <Wheat size={16} className="text-brand-600" />
